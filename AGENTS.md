...
每次会话开始读取 `memory-bank/` **时间线层**文件（projectbrief / techContext / systemPatterns / activeContext / progress / decisions），缺失则创建。

**领域层不一次全读（R29 渐进式披露）**：`memory-bank/domains/` 按需加载——先读 `domains/README.md` 判归属，再按当前判断取用对应领域文件，最后回源核对。
...
| 版本交付 | `progress.md` | 版本里程碑 +1（R16 账本口径，不堆过程细节） |
...
`../ctt-server`（后端）与 `../ctt-web`（前端）均为**只读**关联项目：涉及云同步/API 对接、DTO 契约、认证协议、响应格式变更时**主动读取** ctt-server 对应源码验证契约（R10），不猜测接口形状。云同步已实现（0.20.1：LWW 冲突裁决 + 游标 + 服务端分页 + 批量 upsert），协议事实与插件侧契约见 `memory-bank/domains/sync-protocol/`。

**读取权限**：只读研究契约（DTO/端点/认证/错误码）是允许且必须的——用于验证插件侧实现，**绝不自行修改关联项目**。

**严禁修改关联项目任何文件**（含源码/测试/文档/版本号），无论任何理由。

**需求报告流程**（当工作依赖关联项目改动时）：
1. **绝不自行修改关联项目**——哪怕实现缺失、契约不符，也只在插件侧适配或提需求
2. 输出**可复制的需求报告**（Markdown 文件，可直接转交该项目负责人）：现状 / 期望行为 / 理由 / 影响面 / 验收标准
3. 报告交用户，由用户联系该项目负责人开发（AI 不跨项目实施）
4. 负责人开发完成后结果反馈用户 → 用户转达 → 插件侧对接（仍不碰关联项目）
...
**一条消息中的提交/推送指令只覆盖该消息【指名的那些变更】，提交完毕立即闭合。前瞻动词（"继续/接着做/然后实施 X"）不携带提交授权**——它只授权开发动作。

| 消息 | 授权范围 | 闭合点 |
|---|---|---|
| 「提交这个变更」 | 仅该变更 | 提交后闭合 |
| 「提交并推送，然后继续开发 X」 | 该变更 commit + push | **push 后闭合**；X 开发完需重新授权 |
| 「实施 A、B、C 三批」（无提交字样） | 无提交授权 | 全部实施完，报告 + 待授权 |

每批实施完成后一律 **报告 + 「待授权提交」+ 停**，批次之间不得连续提交。
...
**机械判定法（防自我说服）**：动手前先在本回合用户消息里**字面搜索** `提交` / `commit` / `推送` / `push`。命中 → 可提交（且严格限于其指名的变更）；未命中 → **禁止提交**，改为报告 + 收尾写「待授权提交」后停止。不得以"上一轮授权过"、"已在流程中"、"用户之前说要提交"为由提交。
...
**核心原则：记忆可优化、可归档，但禁止直接删除。** 旧内容（超期/超行数/被新内容取代）视为冷数据——未来可能彻底不用，但无法保证 100% 不用，必须归档而非删除。

| 文件 | 超限处理 |
|---|---|
| activeContext.md | 保留最近30天；>90天前或超200行 → 归档 |
| progress.md | 已完成项 → 归档 |
| systemPatterns.md | 合并相似模式；被取代的旧模式 → 归档 |
| techContext.md | 过时配置 → 归档（不删除） |

**归档流程**：
1. 归档位置：`memory-bank/archive/YYYY-MM-DD-<topic>.md`（目录不存在时创建）；归档文件**只写不改**，头部标注 `> 冷数据归档（R16）。仅供回溯，不再更新。`
2. 触发：文件超 200 行、条目超过保留期、内容被新规则/新实现取代
3. 归档内容保留原文；主文件仅保留一句摘要 + 指向 archive 文件的链接（保持主文件精简可读）
4. **完整性校验**：归档前后总行数差 = 归档文件头行数（零丢失）
5. 只有明确无关且经用户确认的内容才可彻底删除；删除前先确认无引用

**行数超限但无超期条目时**（规则缝隙）：说明当前工作集过密——不得归档保留期内的条目（它们是热数据），也**不得删除内容**。顺序：① 压缩条目措辞（合并重复、删冗余定语，信息量不减少）→ ② 将已完结的成组条目移入归档（保留结论、丢弃过程）→ ③ 仍超限则说明阈值需调整，按 R17 提修订。

**progress.md 职责边界**：「什么版本交付了什么」进 progress；「最近具体改了什么」进 activeContext（30 天热窗口）。禁止在 progress 里堆逐条变更——**过期的清单比没有清单更危险**（本项目先例：progress 曾长期停在 C 阶段，把已交付的分页/批量 upsert 等写成未完成，误导新会话）。progress 是**版本里程碑账本**，由提交历史 + `libs.versions.toml` 推导，不手工维护逐条变更。
...
**核心原则：知识按「领域」沉淀，不按「时间」堆积。** 时间线层（activeContext/progress）只回答"最近发生了什么"；跨轮次可复用的判断（协议规则、统计口径、追踪约束、图表规范）沉淀到 `memory-bank/domains/<domain>/`。

**第一层永远是领域**（业务/技术能力面，如 `sync-protocol`、`stats-aggregation`），禁止按文档类型建第一层（`principles/`、`practices/` 这类全局扁平目录）。

**每领域五件套**（缺一不可，建立即填实，禁止占位）：

| 文件 | 职责 | 判定标准 |
|---|---|---|
| `meta.md` | 领域边界（in/out of scope）、代码路径、阅读起点、术语表、核对基线 | 只看这一个就知道"归哪、从哪看起" |
| `principles.md` | 不变量与第一性原理（决策依据、冲突裁决） | 能用来裁决新情况 |
| `scenarios.md` | 触发场景 → 判断 → 动作 | 遇到 X 该怎么做，不用重新推理 |
| `practices.md` | 具体做法、参数、代码模式、踩坑与规避 | 可直接照做，含反例与"为什么" |
| `references.md` | 外部契约、端点、字段、路径、常量 | 事实性查表，不含判断 |

**五件套是覆盖约束**：它规定「解决本领域一个问题，**至少**要理解哪些方面」。某方面没有沉淀 → 说明缺失，不是「不需要」。

**渐进式披露（强制 - 不一次全读）**：`meta.md` 判归属 → `scenarios.md`/`principles.md` 定判断 → `practices.md` 拿做法 → `references.md` 查事实 → 回源核对。一次性读完整个领域树是反模式。

**回源（强制 - 不同事实回不同来源）**：

| 事实类型 | 回源到 |
|---|---|
| 本仓库行为（统计口径、追踪逻辑、图表渲染） | `src/` 代码 + 配置 |
| 服务端契约（端点/字段/错误码/语义） | `../ctt-server` 源码（只读，R3） |
| UI 实际呈现（图表/状态栏外观） | 真实运行（runIde 验收），声明的代码不是证据 |
| 用户/产品意图 | 用户本人的表述；禁止从实现反推「需求本来就是这样」 |
| 历史原因 | `git log` / `memory-bank/archive/` |

禁止两种推定：①「代码这么实现了 → 它必然是正确的业务规则」②「旧文档这么写过 → 可以忽略代码已经变了」。

**维护机制**：
- **增量**：契约/结构变更（同步协议、DTO、SQLite schema、统计口径、图表层级）时**同一轮**更新受影响领域文件
- **校准**：重大改造或复盘后回看领域文件是否漂移；`../ctt-server` 版本变化后复核 `sync-protocol` 契约事实
- **高风险知识**（同步协议、数据模型、schema、统计口径）**语义确认归人**；自动化只负责「发现变化、生成候选、阻止遗漏」，禁止「代码一变就自动覆盖知识」

**元数据**：关键事实标注来源（代码路径 / 提交 / 外部文件+符号）；每个领域 `meta.md` 声明**核对基线**（最后对照来源核对的日期与版本、覆盖范围、已知漂移），内容早于当前来源版本的如实说明，不得暗示刚核对过。

**骨架优先于检索**：领域文件是骨架，定义必须理解的核心事实；检索（grep / 历史文档）只用于**补证**与长尾发现，补证结论经确认后回写领域文件。

**生长规则**：按需建档（宁可少而实，禁止为凑结构建空领域）；`domains/README.md` 索引与目录同步；横切规范留 `systemPatterns.md`，领域专属判断进领域文件，**不得两处重复**。

**红线**：❌ 占位文件 / `TODO: fill` / 空章节 ❌ 把领域文件当 changelog 用 ❌ 与代码/契约不一致的表述（涉契约先只读核对源码）❌ 把推断当事实写入（不确定标「待确认」）❌ 代码一变就自动覆盖高风险知识；单文件仍受 ≤200 行约束。

### R30: AI 产物位置（强制）

**`docs/`（尚未建立，如未来创建）只放面向用户的项目文档**；AI 工作产物一律放 `.omp/`（已 gitignore，不进仓库）。

| 产物 | 位置 |
|---|---|
| 实施计划 | `.omp/plans/<feature>-plan.md`（不带日期，日期写文件内 `Date:` 字段）——**本项目已定义计划位置，覆盖全局约定的 `docs/plans/` 回退** |
| 关联项目需求报告 | `.omp/<topic>-requirement.md` |
| 交付报告 / 验收指南 | `.omp/delivery-report-<topic>.md`、`.omp/manual-qa-<topic>.md` |
| Agent 记忆 | `memory-bank/`（受 R1/R2/R16/R29 治理，**需要提交**） |
| 设计文档（历史快照） | `memory-bank/docs/`（只增不改；当前有效结论沉淀进 R29 领域层） |

红线：禁止把 AI 工作产物写进 README / 源码目录 / `docs/`（面向用户区）。

## 执行流程

会话开始 → 读 memory-bank 时间线层（领域层按需，R29）→ 创建todo（如需）→ 处理请求 → 清理临时文件 → 更新记忆 → 检查行数修剪 → 代码修改+编辑验证 → 提交前审查+Git授权+版本号
...
**时间线层**（回答"最近发生了什么"，R1 会话开始全读）：

`memory-bank/`：projectbrief.md（目标）、techContext.md（技术栈）、systemPatterns.md（横切规范）、activeContext.md（当前，30 天热窗口）、progress.md（版本里程碑账本）、implementation-plan.md（阶段计划）、decisions.md（决策记录）、`memory-bank/docs/`（设计文档历史快照）、`memory-bank/archive/`（R16 冷归档，**唯一豁免 200 行限制**）

**领域层**（回答"这里什么是真的、该怎么做"，R29 治理，按需读取）：

`memory-bank/domains/<domain>/` — 每领域五件套 `meta.md`（边界/路径/术语/核对基线）/ `principles.md`（不变量）/ `scenarios.md`（触发→判断→动作）/ `practices.md`（做法与坑）/ `references.md`（事实查表）；入口 `domains/README.md`（操作规程，按需建档，无领域时不存在）
