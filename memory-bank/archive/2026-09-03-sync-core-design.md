> 冷数据归档（R16）。仅供回溯，不再更新。
> 来源：`memory-bank/docs/SYNC-CORE-DESIGN.md`（C 阶段设计，v0.49.0 时代）。
> 取代者：`memory-bank/domains/sync-protocol/`（当前有效结论，0.20.1）。二者不同之处：本文没有分页（hasMore）、没有批量 upsert、没有墓碑 lifting、测试数 85（现 119）。

# C 阶段设计文档：同步核心（Pull / Push / 游标）（归档原文）
...
| **软删行永不复活** | `findBySessionUuids` 过滤 is_deleted=0；upsert 的 ON CONFLICT 不碰 is_deleted（**已被 0.20.0 取代：服务器 upsert 现在 lifting 墓碑**） |
